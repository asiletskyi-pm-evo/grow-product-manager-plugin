...
- **Fallback — a three-level degradation chain.** The level is decided **once, at the skill's Step 0-host** (`host-profiles.md` §3) from the capabilities actually observed in the session, and carried for the whole run — never re-derived before each batch:
