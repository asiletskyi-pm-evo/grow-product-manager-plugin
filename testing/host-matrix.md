...
| `product-landscape` | full | degraded | degraded | degraded | scan needs SHELL (else the user pastes the app list); optional connectors observed per session; storage: session mode, export at the end; config/storage: via connector or the repo, never assume prior state |
