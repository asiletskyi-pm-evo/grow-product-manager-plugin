...
| 2026-09-08 | Claude — in-context descriptions-only simulation, single run (headless `claude -p --plugin-dir` was not logged in on the machine; author-biased, to be replaced by a real run before v3.0.0 ships) | A–L 100 | None. |
| (fill after each run) | | | |
