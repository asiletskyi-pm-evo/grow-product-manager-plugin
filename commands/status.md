...
1. **Plugin version** — read `${CLAUDE_PLUGIN_ROOT}/.claude-plugin/plugin.json` → `version`.
...
6. With `--verbose`: also list the agents (`${CLAUDE_PLUGIN_ROOT}/agents/*.md`) and commands (`${CLAUDE_PLUGIN_ROOT}/commands/*.md`) this plugin ships, and the skill versions from `skills/*/SKILL.md` frontmatter.
